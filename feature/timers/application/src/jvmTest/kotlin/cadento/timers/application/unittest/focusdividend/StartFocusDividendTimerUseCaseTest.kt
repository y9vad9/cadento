...
class StartFocusDividendTimerUseCaseTest {
    private val repository: TimerRepository = mockk()
    private val clock: Clock = mockk()
    private val useCase = StartFocusDividendTimerUseCase(repository, clock)

    @Test
    fun `execute from terminated state returns Success`() = runTest {
        // GIVEN
        val id = TimerId(Uuid.random())
        val now = Instant.parse("2024-01-01T00:00:00Z")
        val timer = createTimer(
            id, 
            FocusDividendTimerState.Terminated(now.minus(kotlin.time.Duration.parse("1h")), null)
        )
        ...
        // THEN
        assertIs<StartFocusDividendTimerUseCase.Result.Success>(result)
    }
    ...
    @Test
    fun `execute when already earning returns Success without update`() = runTest {
        // GIVEN
        val id = TimerId(Uuid.random())
        val now = Instant.parse("2024-01-01T00:00:00Z")
    ...
    fun `execute with non existing timer returns TimerNotFound`() = runTest {
        ...
        // THEN
        assertIs<StartFocusDividendTimerUseCase.Result.TimerNotFound>(result)
    }
    ...
    fun `execute with get repository failure returns Error`() = runTest {
        ...
        // THEN
        assertIs<StartFocusDividendTimerUseCase.Result.Error>(result)
