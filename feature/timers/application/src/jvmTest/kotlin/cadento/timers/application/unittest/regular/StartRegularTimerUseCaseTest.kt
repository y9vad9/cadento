...
class StartRegularTimerUseCaseTest {
    private val repository: TimerRepository = mockk()
    private val clock: Clock = mockk()
    private val useCase = StartRegularTimerUseCase(repository, clock)
    ...
    fun `execute with inactive timer returns Success`() = runTest {
        ...
        // THEN
        assertIs<StartRegularTimerUseCase.Result.Success>(result)
    }

    @Test
    fun `execute with already active timer returns Success without change`() = runTest {
        // GIVEN
        val id = TimerId(Uuid.random())
        val now = Instant.parse("2024-01-01T00:00:00Z")
        val timer = RegularTimer(
    ...
    fun `execute with non existing timer returns TimerNotFound`() = runTest {
        ...
        // THEN
        assertIs<StartRegularTimerUseCase.Result.TimerNotFound>(result)
    }
    ...
    fun `execute with get failure returns Error`() = runTest {
        ...
        // THEN
        assertIs<StartRegularTimerUseCase.Result.Error>(result)
